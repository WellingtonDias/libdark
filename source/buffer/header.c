...
DKcharacter *BufferEndian[] = {"AUTO","LITTLE","BIG"};

void dkBuffer_debug(DKbuffer *BUFFER,DKcharacter *LABEL)
{
	safe_start(BUFFER);
	printf("BUFFER { endian: %lli, offset: %lli, size: %lli, capacity: %lli, source: ",BufferEndian[BUFFER->endian],BUFFER->offset,(BUFFER->block).size,(BUFFER->block).capacity);
