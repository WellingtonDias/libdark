void dkOrdinal_merge(DKordinal *ORDINAL,DKssize INDEX,DKordinal *SOURCE,DKssize START,DKssize END)
{
	...
	block_merge(ORDINAL->block,DKvalue,INDEX,SOURCE->block,START,END);
...
{
	safe_start(ORDINAL);
	block_clear(ORDINAL->block,DKvalue);
	safe_endExit(ORDINAL);
};
...
DKboolean dkOrdinal_compare(DKordinal *ORDINAL1,DKordinal *ORDINAL2)
{
	...
	DKboolean comparison = block_compare(ORDINAL1->block,ORDINAL2->block,DKvalue);
	mutex_unlock(ORDINAL2->mutex);
	safe_endReturnValue(ORDINAL1,comparison);
};
