...
{
	#local mtx_t *lock;
	if (!(lock = malloc(sizeof(mtx_t)))) error_throwBreak("MEMORY: malloc");
	if (mtx_init(lock,mtx_plain) == thrd_error) error_throwBreak("MUTEX: init");
	MUTEX.lock = lock;
};
...
#routine mutex_setLock(MUTEX,LOCK)
{
	if (LOCK)
	{
		...
		else error_throwBreak("invalid LOCK");
	}
